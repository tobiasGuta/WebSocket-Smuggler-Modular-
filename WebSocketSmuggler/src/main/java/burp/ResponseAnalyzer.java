    ...
    public static ResponseAnalysis analyze(byte[] rawBytes) {
        ...
        if (codes.size() >= 2) {
            ...
            if (secondIsSuccess) {
                ...
                // Both responses have the same error code — proxy parsed them individually
                status = "Pipelining (Safe)";
            } else {
                // Two different error codes — the smuggled request reached the backend
                status = "Potential Smuggling (" + code1 + " → " + code2 + ")";
            }
